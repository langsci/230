 ...
 \begin{exe}
	...
	\begin{xlist}
		\ex \label{ex:tassen} \object{Er zerbrach bei der Feier eine sehr teure Vase. \textbf{Dieses/Das Missgeschick} blieb noch lange in Erinnerung.} 
			\ex \label{ex:entwicklung} \object{Die Arbeitslosigkeit steigt, die Inflation schreitet fort, die Wirtschaft ist rückläufig. \textbf{Diese/}??\textbf{Die Entwicklung} ist gefährlich.} 
		\end{xlist}
\end{exe}
...
\begin{exe}
	\ex \label{ex:diskurs-deikt-def}  \object{Anstatt für seine Prüfungen zu lernen, ist Robert im Kino gewesen. \textbf{Diese\slash}?\textbf{Die Nachlässigkeit} könnte ihn das Vordiplom} kosten. \\(diskurs-situativer Gebrauch)
	 \end{exe}
...
\begin{exe}
	\ex \label{ex:asso3}   
	\begin{xlist}
		\ex \label{ex:meronymie} \object{Dort steht ein Haus.} \\ \object{\textbf{Das Fenster/die Tür/der Balkon} ist blau gestrichen.} (Teil-Gan"-zes-Re"-la"-tion)
...
\begin{itemize} 
		\item[a)] \label{etab} NP mit etablierendem Relativsatz: \\ \object{Was ist mit Bill los? -- \textbf{Die Frau}, mit der er ausgegangen ist, war gemein zu ihm.} 
...
Als weitere \object{modifier}, die dazu führen, dass eine NP von einem Definitartikel begleitet werden muss, nennt  \textcite[148,228ff.]{Hawkins1978} \object{same}, \object{only}, \object{next} und \object{first} sowie Superlative, vgl. die Beispiele in \REF{ex:mod}; vgl. zur Diskussion \textcite[9]{Lyons1999}.
 ...
 \begin{exe}
	...
	\begin{xlist}
		...
		\ex \label{ex:next} \object{\textbf{Der nächste Anrufer} gewinnt ein Auto.} 
		\ex \label{ex:first} \object{\textbf{Der erste Kunde} musste lange anstehen.}  
...
\begin{exe}
	\ex \label{ex:gener2} 
	\begin{xlist}
		\ex \label{ex:frz}  \object{\textbf{Les chats} sont des mammifères.}
...
\lsptoprule
Gebrauch & Beispiel & Funktion & Ident. durch & \multicolumn{3}{c}{Artikel}\\\cmidrule(lr){5-7}
